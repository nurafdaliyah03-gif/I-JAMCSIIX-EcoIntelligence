...
def load_geojson():
    try:
        ...
        for feature in res['features']:
            ...
            feature['properties']['PROV_KEY'] = "DKI JAKARTA" if "JAKARTA" in nama else "DI YOGYAKARTA" if "YOGYAKARTA" in nama else nama
        return res
    except: return None
...
if st.session_state.page == "Portal":
    ...
    with c_up2:
        ...
        if up_file:
            ...
            if 'PROVINSI' in raw_df.columns: raw_df['PROVINSI'] = raw_df['PROVINSI'].astype(str).str.strip().str.upper()
    ...
    if st.session_state.page == "Dashboard" and st.session_state.df is not None:
        ...
        with col_f2: sel_prov = st.selectbox("Fokus Wilayah:", ["Semua Provinsi"] + sorted(df['PROVINSI'].unique().tolist()))
        ...
        with cr:
            ...
            fig2 = px.scatter(df_filt, x=cols_x[var_x], y=col_y, trendline="ols", hover_name="PROVINSI")
        ...
        prov_target = st.selectbox("Fokus Wilayah:", sorted(df['PROVINSI'].unique()))
        hist = df[df['PROVINSI'] == prov_target].sort_values('TAHUN')
        
        # Perbaikan Error Baris 281
        raw_weights = np.random.dirichlet([5, 3.5, 2, 1])
        
        # ... (Logika Prediksi lainnya tetap sama) ...
        ...
        with rc1:
            st.markdown("""<div class='research-card'><h4>🎯 Tujuan Penelitian</h4><ul><li>Menerapkan pendekatan data longitudinal dan model hibrida Mixed Effects Random Forest (MERF) untuk menangkap tren perubahan waktu sekaligus karakteristik spasial.</li><li>Membangun aplikasi web interaktif ForestGuard sebagai media visualisasi spasial-temporal dan sistem prediksi risiko deforestasi.</li></ul></div>""", unsafe_allow_html=True)
        with rc2:
            ...
            st.markdown("""<p><b>Keterangan:</b><br>• y_i: Vektor nilai variabel respon (Tree Cover Loss) untuk provinsi ke-i.<br>• f(X_i): Fixed effects via Random Forest.<br>• Z_i: Matriks desain random effects.<br>• b_i: Vektor penyimpangan acak.<br>• \varepsilon_i: Vektor error acak.</p></div>""", unsafe_allow_html=True)
        
        st.markdown("""<div style='background: linear-gradient(135deg, #7f1d1d 0%, #450a0a 100%); padding: 25px; border-radius: 15px; border: 1px solid #ef4444;'>
            <h5>⚠️ Batasan Penelitian & Disclaimer</h5>
            <ul><li>Ketergantungan Data Historis: Tidak membaca perubahan mendadak/kebijakan baru.</li>
            <li>Optimal Jangka Pendek: Rentan akumulasi error pada jangka panjang.</li>
            <li>Efek Wilayah Baru: Mengabaikan efek acak pada provinsi baru (b_i=0).</li>
            <li>Cakupan Variabel Makro: Tidak memperhitungkan faktor pemicu eksternal mendadak (exogenous shocks) di luar variabel terdata.</li>
            <li>Resolusi Spasial Makro: Bukan untuk deteksi real-time koordinat petak hutan.</li></ul></div>""", unsafe_allow_html=True)
