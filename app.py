...
if st.session_state.page == "Portal":
    ...
    if st.session_state.page == "Dashboard":
        # ... kode dashboard Anda di sini ...
        st.write("Dashboard aktif")

    # --- BAGIAN PREDIKSI (YANG DIUBAH DENGAN FITUR UPLOAD) ---
        ...
        with c_filter:
            prov_target = st.selectbox("Pilih Wilayah untuk Grafik Tren:", sorted(st.session_state.df['PROVINSI'].unique()))
        with c_upload:
        ...
        last_yr = df_use['TAHUN'].max()
        
        with cl:
            st.markdown("<h4 style='color: #facc15;'>📄 TABEL ESTIMASI 3 TAHUN (NASIONAL)</h4>", unsafe_allow_html=True)
            national_preds = []
            for p in sorted(df_use['PROVINSI'].unique()):
                last_val = df_use[df_use['PROVINSI'] == p].iloc[-1][col_y]
                for i in range(1, 4):
                    thn = last_yr + i
                    national_preds.append({"Provinsi": p, "Tahun": thn, "Estimasi Loss (Ha)": round(last_val * (1.03 ** i), 2)})
        ...
        with cr:
            st.markdown(f"<h4 style='color: #facc15;'>📊 TREN KEHILANGAN TUTUPAN POHON - {prov_target}</h4>", unsafe_allow_html=True)
            hist = df_use[df_use['PROVINSI'] == prov_target].sort_values('TAHUN').copy()
            hist['Status'] = 'Data Aktual'
            future_data = pd.DataFrame({'TAHUN': [last_yr+1, last_yr+2, last_yr+3], col_y: [hist[col_y].iloc[-1] * (1.03 ** i) for i in range(1, 4)], 'Status': 'Prediksi'})
            df_plot = pd.concat([hist[['TAHUN', col_y, 'Status']], future_data])
            
            fig_pred = px.line(df_plot, x='TAHUN', y=col_y, color='Status', markers=True, color_discrete_map={'Data Aktual': '#22c55e', 'Prediksi': '#ef4444'})
            fig_pred.update_layout(paper_bgcolor='white', legend=dict(orientation="h", yanchor="bottom", y=-0.3, xanchor="center", x=0.5))
            st.plotly_chart(fig_pred, use_container_width=True)

    # --- BAGIAN PENELITIAN (TETAP SAMA) ---
    elif st.session_state.page == "Penelitian":
        # ... kode penelitian Anda di sini ...
        st.write("Penelitian aktif")
