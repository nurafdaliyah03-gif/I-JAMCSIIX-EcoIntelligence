...
if st.session_state.df is None:
    ...
    if lokal_csv:
        ...
        try:
            ...
            # Deteksi otomatis kolom Y dan X
...
<style>
    ...
    .stSelectbox label p { color: #facc15 !important; font-weight: bold !important; }
    ...
    [data-testid="stMetricLabel"] { color: #facc15 !important; font-weight: bold !important; }
...
if st.session_state.page == "Portal":
    ...
    with c1:
        ...
        if st.button("Buka Dashboard", disabled=False, key="btn_dash"):
            set_page("Dashboard")
            st.rerun()
            
    with c2:
        st.markdown("<div class='menu-card'><h1>🧪</h1><h3>Prediksi MERF</h3></div>", unsafe_allow_html=True)
        if st.button("Mulai Prediksi", disabled=False, key="btn_pred"):
            set_page("Prediksi")
            st.rerun()
        ...
        if df is not None:
            ...
            with cl:
                if geojson and not df_filt_year.empty:
                    ...
                    fitur_fit = False if sel_prov == "Semua Provinsi" else "locations"
    ...
    # --- HALAMAN PREDIKSI MERF ---
        ...
        if df is not None:
            ...
            st.error("⚠️ Gagal memuat simulasi data.")
    ...
    # --- HALAMAN PENELITIAN ---
        ...
        with rc1:
            st.markdown("<div class='research-card'><h4>🎯 Tujuan Penelitian</h4><p>Menerapkan pendekatan MERF untuk analisis spasial-temporal deforestasi regional Indonesia.</p></div>", unsafe_allow_html=True)
        with rc2:
